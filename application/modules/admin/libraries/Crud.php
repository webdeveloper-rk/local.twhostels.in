 ...
 * @link		http://www.grocerycrud.com/documentation
 */
class CRUD_Model_Driver extends CRUD_Field_Types {
    ...
    protected function set_default_Model() {
        ...
        $this->basic_model = $ci->CRUD_Model;
    }
    ...
    protected function form_validation() {
        if ($this->form_validation === null) {
            $this->form_validation = new CRUD_Form_validation();
            $ci = &get_instance();
    ...
    protected function get_list() {
        ...
        if ($this->config['crud_paging'] === true) {
            if ($this->limit == null) {
                ...
                $default_per_page = $ci->config->item('crud_default_per_page');
    ...
    protected function upload_file($state_info) {
        if (isset($this->upload_fields[$state_info->field_name])) {
            if ($this->callback_upload === null) {
                ...
                $allowed_files = $ci->config->item('crud_file_upload_allow_file_types');
                $reg_exp = '/(\\.|\\/)(' . $allowed_files . ')$/i';

                $max_file_size_ui = $ci->config->item('crud_file_upload_max_file_size');
 ...
 * LICENSE
 *
 *  CRUD is released with dual licensing, using the GPL v3 (license-gpl3.txt) and the MIT license (license-mit.txt).
 * You don't have to do anything special to choose one license or the other and you don't have to notify anyone which license you are using.
 * Please see the corresponding license file for details of these licenses.
 * You are free to use, modify and distribute this software, but all copyright information must remain.
 *
 ...
 * @version    	1.2
 */
class CRUD_Layout extends CRUD_Model_Driver {
    ...
    public function set_echo_and_die() {
    ...
    protected function get_relation_input($field_info, $value) {
        ...
        $ajax_limitation = $ci->config->item('crud_set_relation_max_data_without_ajax');
 ...
 * @version    	1.2
 */
class CRUD_States extends CRUD_Layout {
 ...
 * @link		http://www.grocerycrud.com/documentation
 */
class CRUD extends CRUD_States {
    ...
    protected $default_javascript_path = 'assets/crud/js';
    protected $default_css_path = 'assets/crud/css';
    protected $default_texteditor_path = 'assets/crud/texteditor';
    protected $default_theme_path = 'assets/crud/themes';
    protected $default_language_path = 'assets/crud/languages';
    ...
    private function _load_language() {
        ...
        if ($this->language === null) {
            $this->language = strtolower($ci->config->item('crud_default_language'));
        }
...
//@todo ui_day, ui_month, ui_year has to be lang strings
        ...
        $date_format = $ci->config->item('crud_date_format');
        switch ($date_format) {
            case 'uk-date':
...
class CRUD_Form_validation extends CI_Form_validation {
