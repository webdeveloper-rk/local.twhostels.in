...
class gc_dependent_select
{
	...
	if (!empty($result))
	{
	    $substr_count = substr_count($this->fields[$field_info->name]['name'], "{");
	    if ($substr_count > 0)
	    {
		preg_match_all('/{(.+?)}/', $this->fields[$field_info->name]['name'], $fields_array);
	    }
		...
		if ($substr_count > 0)
		{
		    $html.= str_replace($fields_array[0], $replace, $this->fields[$field_info->name]['name']);
		}
		else
		{
		    $html.= $item[$this->fields[$field_info->name]['name']];
		}
	...
	$substr_count = substr_count($this->fields[$this->field_name]['name'], "{");
	if ($substr_count > 0)
	{
	    preg_match_all('/{(.+?)}/', $this->fields[$this->field_name]['name'], $fields_array);
	}
	...
	foreach ($db->result_array() as $row)
	{
	    ...
	    if ($substr_count > 0)
	    {
		$array[] = array("value"    => $row[$this->fields[$this->field_name]['id_field']], "property" => str_replace($fields_array[0], $replace, $this->fields[$this->field_name]['name']));
	    }
	    else
	    {
		$array[] = array("value"    => $row[$this->fields[$this->field_name]['id_field']], "property" => $row[$this->fields[$this->field_name]['name']]);
	    }
	    $replace   = NULL;
	};
