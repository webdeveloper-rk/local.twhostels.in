	...
	<?php //} ?>
                ...
                <tbody>
				 ...
				 foreach($rset->result() as $item) { 				 				 if(!in_array($item->item_id,$allowed_items))						continue;									//echo "<pre>";print_r($item);echo "</pre>";die; 				 ?> 				 <tr role="row" class="odd">
                  <td class="sorting_1">
				  <a href='<?php echo site_url();?>consumption_entry/consumption_entryform/<?php echo $item->item_id;?>/<?php echo $session_id;?>'>
				...
				<td><?php 								 if($data_entry_allowed==true || ($this->session->userdata("operator_type")=="CT" && $ses_p['code']==2) ) 	{?> 
                  <a href='<?php echo site_url();?>consumption_entry/consumption_entryform/<?php echo $item->item_id;?>/<?php echo $session_id;?>'><?php if($this->session->userdata("operator_type")=="DEO"){ echo "UPDATE";} elseif($this->session->userdata("operator_type")=="CT") { echo "EDIT";}else { echo "Update";} ?></a>				  								 
				 <?php } ?>   </td>				 
                </tr>
				 <?php } ?>					 
