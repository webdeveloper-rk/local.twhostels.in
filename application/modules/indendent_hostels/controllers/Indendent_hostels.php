...
class Indendent_hostels extends MX_Controller {
	...
	redirect("indendent_hostels/alreadygenerated"); 
...
function indent_schools_list($encoded)
	{
		...
		$schools_names  = array();
		$rs = $this->db->query("select * from schools where district_id=? and school_id=?",array($this->session->userdata("district_id"),$this->session->userdata("school_id")));
		...
		$data['generated_list'] = $this->db->query("select indent_info_id,indent_date,school_id,date_format(indent_date,'%d-%M-%Y') as dp_date from indent where district_id=? and school_id=?   and indent_info_id=? group by school_id, indent_date ",array($this->session->userdata("district_id"),$this->session->userdata("school_id"),$indent_info_id));
		//echo $this->db->last_query();
