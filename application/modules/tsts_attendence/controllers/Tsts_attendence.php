...
class Tsts_attendence extends MX_Controller {
   	...
   	public function index()
	{
		...
		{
			 $school_attendence[$scrow->ehostel_id]['school_name']  =$scrow->name;
			 $school_attendence[$scrow->ehostel_id]['school_id']  =$scrow->school_id;
			 $school_attendence[$scrow->ehostel_id]['school_code']  =$scrow->school_code;
			 $school_attendence[$scrow->ehostel_id]['cat_1_total']  = 0;
			 $school_attendence[$scrow->ehostel_id]['cat_2_total']  = 0;
			 $school_attendence[$scrow->ehostel_id]['cat_3_total']  = 0;
			 $school_attendence[$scrow->ehostel_id]['total']  = 0;
			 $school_attendence[$scrow->ehostel_id]['classes']  = array();
		}
		 ...
		 foreach($att_list as $key=>$attObj)
		 {
			 $class_numeric_id = intval($attObj->class);
			 
			if($class_numeric_id==0   )
					continue;
			
			if($class_numeric_id >10)
					continue;
			
			$total =  intval($school_attendence[$attObj->orgcode]['total']);
			$total =$total + $attObj->attempts;
			
			
			$school_attendence[$attObj->orgcode]['total'] = $total ;
			$school_attendence[$attObj->orgcode]['classes'][$attObj->class] = $attObj->attempts ; 
			if(in_array($attObj->class,$cat_1_classes)){
							$cat_1_total =  intval($school_attendence[$attObj->orgcode]['cat_1_total']);
							$school_attendence[$attObj->orgcode]['cat_1_total']= $cat_1_total + $attObj->attempts;
			}else if(in_array($attObj->class,$cat_2_classes)){
		...
		// print_a($school_attendence,0);
		 ...
		 $html_text =  "<table style='font-family: Verdana, Geneva, sans-serif;font-size: 11px;'><tr class='sticky'>
					...
					<th style=' $th_style'>ehostel id</th>
					...
					<th style=' $th_style'>School DDO Code</th>
	...
	public function CallAPI($date)
	{
		//echo 'Curl: ', function_exists('curl_version') ? 'Enabled' : 'Disabled'; 
			...
			$method ="POST";
		$url =  'https://attendance.telangana.gov.in/attendeesapi/api/attendee/gettxnattempts?date='.$date;
		$apikey =  '6614c747-e07d-4336-9435-a8021190b379';
		...
		switch ($method)
		{
			case "POST": 
				curl_setopt($curl, CURLOPT_POST, 1);
				 ...
				 curl_setopt($curl, CURLOPT_POSTFIELDS, array('a123'=>'12345','date'=>$date));
				break;
			case "PUT":
