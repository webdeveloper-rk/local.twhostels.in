	...
	function update($encoded_data=null)
	{
		...
		$decoded_data  = $this->ci_jwt->jwt_web_decode($encoded_data );
		 ...
		 $date= $decoded_data->date;
		  ...
		  $todate1 =  $date;
			  $check_date_in_date =$this->db->query("select ? between '2020-04-16' and CURRENT_DATE() as res",array($todate1))->row()->res;
				if($check_date_in_date==0)
				{
					$today_text = date('d-M-Y');
						$this->session->set_flashdata('message', '<div class="alert alert-danger">Date Should be between 16-04-2020 and '.$today_text.' </div>');
		...
		if($this->form_validation->run() == true )
		{
		 ...
		 $update_data = array( 'purchase_quantity'=>floatval($this->input->post('pqty')),
			...
			$uploaded_file = 'dummy.jpg';//$upload_data['upload_data']['file_name'];
	  ...
	  private function free_distribution($entry_id_e,$uploaded_file)
	{
		...
		//	 print_a($entry_data ,1);
			...
			if($free_distribution_rs->num_rows()==0)
			{
				...
				//$ins['whom_description'] = $this->input->post("whom_description"); 
				...
				$up_data['to_whom'] = $this->input->post("whom");
				...
				$up_data['whom_description'] = $this->input->post("whom_description");
