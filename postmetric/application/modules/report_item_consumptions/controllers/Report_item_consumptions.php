...
class Report_item_consumptions extends MX_Controller {
	...
	*****************************************************************************/
	function index() {
			...
			$items_rs = $this->db->query("select concat(telugu_name,'-',item_name) as name from items where item_id=?",array($item_id));
			 if($items_rs->num_rows()==0)
			  ...
			  $sql  = "select 
							...
							from balance_sheet where month(entry_date)=? and YEAR(entry_date)=?  $condition and item_id=? group by entry_date order by entry_date asc ";
        ...
        $data["module"] = "report_item_consumptions";
